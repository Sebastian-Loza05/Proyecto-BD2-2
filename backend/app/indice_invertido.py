...
def retrieve_top_k(query, k, num_blocks, num_docs, idf_values):
    ...
    for block_num in range(num_blocks):
        block_similarities = cosine(query, block_num, num_docs)
...
def cosine(query, block_num, num_docs):
    with open(f"block_{block_num}.pkl", "rb") as file:
        block_index = pickle.load(file)
    ...
    for term in query_terms:
        if term in block_index:
            df_t = len(block_index[term])
            idf = score_idf(num_docs, df_t)
            for doc_id in block_index[term]:
                tf = block_index[term].count(doc_id)
                doc_scores[doc_id] += score_tf(tf) * idf
                
    # Ponderación adicional para la coincidencia de múltiples términos
    for doc_id in doc_scores:
        terms_matched = sum([1 for term in query_terms if term in docs[doc_id]])
